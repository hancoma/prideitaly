...
function json_call(reg_id) {
      ...
      // gcm reg_id 등록  
         $.post("http://m.prideitaly.net/gcm_reg_app.php",
   {
    reg_id:reg_id,
    deviceid:deviceid
   },
...
function left_menu() {
     ...
     $( document ).ready(function() {
                    $.post("http://m.prideitaly.net/left_menu.php",
               {
...
function goHref(url) {
  ...
  url="http://m.prideitaly.net/"+url+"&uuid="+uuid;
...
function getpage(uuid,page) {
    ...
    var uuid=uuid;
    var page=page;
    var url="http://m.prideitaly.net/"+page+".php";
 $.post(url,
   {
...
function check_uuid (deviceid) {
        var deviceid=deviceid;
                     ...
                     $.post("http://m.prideitaly.net/check_uuid_app.php",
       {
                deviceid:deviceid
       },
       function(data){
...
function onConfirm(buttonIndex) {
    ...
    if (btn==1) {
      ...
      $.post("http://m.prideitaly.net/logout_app.php",
       {
                deviceid:uuid
       },
       function(data){
...
sourceType: navigator.camera.PictureSourceType.PHOTOLIBRARY
    ...
    function uploadPhoto_photo(imageURI) {
      var deviceid=device.uuid;
        ...
        ft.upload(imageURI, "http://m.prideitaly.net/upload_org.php", win_photo, fail, options);
    }
...
function showPosition(position)
  ...
  if (x) {
   $.post("http://m.prideitaly.net/gps_update_app.php",
   {
    y:y,
    x:x,
    uuid:uuid
...
right_menu();
 ...
 $( document ).ready(function() {
  ...
  $.post("http://m.prideitaly.net/right_menu_app.php",
   {
      deviceid:deviceid
